...
Model::Model(crstring file, crint num)
{
	mNumDraw = num;
	Load(file.c_str());
}
...
void Model::Draw()
{
	...
	for (unsigned int i = 0; i < mSubsetNum; i++)
	{
		// マテリアル設定
		Renderer::SetMaterial(mSubsetArray[i].Material.Material);

		if (mSubsetArray[i].Material.Texture) {
			// テクスチャ設定
			Renderer::GetDeviceContext()->PSSetShaderResources(0, 1, &mSubsetArray[i].Material.Texture);
		}

		// ポリゴン描画
		Renderer::GetDeviceContext()->DrawIndexed(mSubsetArray[i].IndexNum, mSubsetArray[i].StartIndex, 0);
	}

}

void Model::Draw(cruint num)
{
	...
	for (unsigned int i = 0; i < mSubsetNum; i++)
	{
		...
		// ポリゴン描画
		Renderer::GetDeviceContext()->DrawIndexedInstanced(mSubsetArray[i].IndexNum, num, mSubsetArray[i].StartIndex, 0, mSubsetArray[i].StartIndex);
...
void Model::NumDraw()
{
	...
	for (unsigned int i = 0; i < mSubsetNum; i++)
	{
		...
		// ポリゴン描画
		Renderer::GetDeviceContext()->DrawInstanced(mNumVertex, mNumDraw, mSubsetArray[i].StartIndex, 0);
...
void Model::Load(const char* FileName)
{
	...
	// サブセット設定
	{
		mSubsetArray = new SUBSET[model.SubsetNum];
		mSubsetNum = model.SubsetNum;

		for (unsigned int i = 0; i < model.SubsetNum; i++)
		{
			mSubsetArray[i].StartIndex = model.SubsetArray[i].StartIndex;
			mSubsetArray[i].IndexNum = model.SubsetArray[i].IndexNum;

			mSubsetArray[i].Material.Material = model.SubsetArray[i].Material.Material;

			mSubsetArray[i].Material.Texture = NULL;

			D3DX11CreateShaderResourceViewFromFile(Renderer::GetDevice(),
				model.SubsetArray[i].Material.TextureName,
				NULL,
				NULL,
				&mSubsetArray[i].Material.Texture,
				NULL);
...
void Model::Load(const char* file, const char* tex)
{
	MODEL model;
	LoadObj(file, &model);
...
void Model::Unload()
{
	...
	{
		if (mSubsetArray[i].Material.Texture) {
			mSubsetArray[i].Material.Texture->Release();
		}
	}
